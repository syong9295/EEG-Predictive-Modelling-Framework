...
if not(isfolder('artifact_processing\artifact_statistics'))
    mkdir('artifact_processing\artifact_statistics');
end

fileID = fopen('artifact_processing\artifact_statistics\artifacts_list.txt', 'w+');
...
% msg = strcat('total number of channels with artifacts (all channels) :', int2str(artifact_counter), ' out of 14350', '\n');
% fprintf(msg)

% message if we use 2 channels (af3 and af4)
msg2 = strcat('total number of channels with artifacts (af3 & af4) :', int2str(artifact_counter), ' out of 2050', '\n');
fprintf(msg2)
