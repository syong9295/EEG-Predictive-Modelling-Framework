...
folder_content = dir("datasets\artifact_free_data_labels\*.lab");
...
for i = 1 : data_no % for each of the label file
    path = strcat('datasets\artifact_free_data_labels\', folder_content(i).name);
    fileID = fopen(path);
...
fileID = fopen('artifact_processing\artifact_statistics\artifact_free_data_class_distribution.txt', 'w+');
...
save('variable_files\artifact_free_data_class_distribution.mat', 'like_counter', 'dislike_counter', 'label_index');

disp("artifact_free_data_class_distribution.txt has been computed.")
