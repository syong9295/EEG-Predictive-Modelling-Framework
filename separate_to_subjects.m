...
if not(isfolder('features_seprated_by_subjects\'))
    mkdir('features_seprated_by_subjects\');
end

if not(isfolder('features_seprated_by_product_no\'))
    mkdir('features_seprated_by_product_no\');
end

for i = 1 : length(sub_feature)
   sub_name = sub_feature(i).name;
   filename = strcat("features_seprated_by_subjects\", sub_name, ".csv");
   writematrix(sub_feature(i).features, filename); 
end

for i = 1 : length(prod_feature)
   prod_no = prod_feature(i).no;
   filename = strcat("features_seprated_by_product_no\", prod_no, ".csv");
